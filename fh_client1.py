#!/bin/env -S pyboots --html http://automationmd/virtualenv/neo/requirements.txt  --require 3.12 --rebuild-on-changes

"""
FastHTML Client Application 1 - JWT SSO Example
"""
            ...
            Container(
                Article(
                    ...
                    Div(
                        ...
                        style="background: #d4edda; padding: 20px; border-radius: 5px; margin: 20px 0;"
                    ),
                    Div(
                    ...
                    P(
                        "Try accessing ",
                        A("Client App 2", href="http://localhost:8002", target="_blank"),
    ...
    return Main(
        Container(
            Article(
                H1("🎯 Client Application 1"),
                Div(
                    P("Welcome to Client Application 1. Please sign in to continue."),
                    style="background: #f8f9fa; padding: 20px; border-radius: 5px; margin: 20px 0;"
                ),
                A("Sign in with SSO", href="/login", role="button"),
                P(
                    "This is a demo client application that uses JWT-based SSO for authentication.",
                    style="margin-top: 40px; color: #666;"
                ),
                Div(
                    H4("How JWT SSO Works:"),
                    Ol(
                        Li("Click 'Sign in with SSO' to redirect to SSO server"),
                        Li("SSO server authenticates and creates a JWT token"),
                        Li("Client app receives JWT and validates it locally"),
                        Li("No need to call SSO server for every request!"),
                        Li("JWT contains user info and is cryptographically signed")
                    ),
                    style="background: #e7f3ff; padding: 15px; border-radius: 5px; margin-top: 20px;"
                )
            )
        ),
        cls="container"
    ...
    return Main(
        Container(
            Article(
                ...
                Div(
                    P("This is a protected resource in Client Application 1."),
                    ...
                    style="background: #fff3cd; padding: 20px; border-radius: 5px; margin: 20px 0;"
                ),
...
if __name__ == "__main__":
    print("=" * 60)
    print("🎯 Client App 1 (FastHTML + JWT) Starting")
    print("=" * 60)
